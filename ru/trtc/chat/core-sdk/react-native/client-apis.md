...
`TencentCloudChat` — это пространство имён Chat JavaScript SDK, которое предоставляет статический метод `create()` для создания экземпляров SDK, константу события `EVENT`, константу типа `TYPES` и константу сигнализации `TSignaling`.
...
| Термин | Описание |
| --- | --- |
| [Message](https://trtc.io/document/47990?platform=javascript&product=chat&menulabel=sdk) | `Message` обозначает содержимое, которое будет отправлено, и содержит несколько атрибутов, определяющих, являетесь ли вы отправителем, учётную запись отправителя, время создания сообщения и так далее. |
| [Conversation](https://trtc.io/document/48328?platform=javascript&product=chat&menulabel=sdk) | Доступны два типа диалога: C2C (Client to Client): Беседа один-на-один, включающая только двух участников. GROUP: Групповой чат, включающий более двух участников. |
| `Profile` | `Profile` описывает основную информацию о пользователе, включая псевдоним, пол, личную подпись и URL фотографии профиля. |
| `Friend` | `Friend` описывает основную информацию о друге, включая заметки и список друзей. |
| `FriendApplication` | `FriendApplication` описывает основную информацию о запросе дружбы, включая источник друга и заметки. |
| `FriendGroup` | `FriendGroup` описывает основную информацию о группе друзей, включая название группы друзей и участников. |
| `Group` | `Group` представляет систему связи для группового чата, включая рабочую группу, открытую группу, группу встреч, аудио-видео группу и сообщество. |
| `GroupMember` | `GroupMember` обозначает основную информацию о каждом члене группы, например ID, псевдоним, роль и время присоединения к группе. |
| `Group tip` | Подсказка группы создаётся при возникновении события, такого как добавление или удаление члена группы. Сторона доступа может настроить, отображать ли подсказки группы членам группы. |
| `Group system message` | Например, когда пользователь запрашивает присоединение к группе, администратор группы получает системное сообщение. После того как администратор принимает или отклоняет запрос, Chat SDK возвращает результат стороне доступа, которая затем отображает результат пользователю. |
...
| API | Описание |
| --- | --- |
| [on](https://trtc.io/document/47967?platform=javascript&product=chat&menulabel=sdk#d39fb50f-f0f2-4b67-9b15-acd7e8608105) | Включает прослушивание события. |
| [off](https://trtc.io/document/47967?platform=javascript&product=chat&menulabel=sdk#c4cb09c5-142a-4d8c-8c03-ea7f446aa844) | Отключает прослушивание события. |
...
| API | Описание |
| --- | --- |
| [destroy](https://trtc.io/document/47967?platform=javascript&product=chat&menulabel=sdk#.E5.8F.8D.E5.88.9D.E5.A7.8B.E5.8C.96) | Завершает работу экземпляра SDK. SDK выполнит выход, отключит постоянное соединение WebSocket и затем освободит ресурсы. |

### Вход

| API | Описание |
| --- | --- |
| [login](https://trtc.io/document/47970?platform=javascript&product=chat&menulabel=sdk#964d8f2e-a986-41fc-aba1-a8bfe6f6c8eb) | Выполняет вход в Chat SDK с использованием userID и userSig. |
| [logout](https://trtc.io/document/47970?platform=javascript&product=chat&menulabel=sdk#c58e1694-051c-404f-acef-1bb45ebc277c) | Выполняет выход из Chat SDK. |
| [isReady](https://trtc.io/document/47970?platform=javascript&product=chat&menulabel=sdk#188c35da-d695-4d33-a192-ae808b597c66) | Обозначает, вошёл ли SDK в готовое состояние или нет. |
...
| [createLocationMessage](https://trtc.io/document/47993?platform=javascript&product=chat&menulabel=sdk#creating-a-geographical-location-message) | Создаёт сообщение с географическим местоположением. |
...
| [downloadMergerMessage](https://trtc.io/document/47993?platform=javascript&product=chat&menulabel=sdk#downloading-a-merged-message) | Скачивает объединённое сообщение. |
| [createForwardMessage](https://trtc.io/document/47993?platform=javascript&product=chat&menulabel=sdk#forwarding-messages-one-by-one) | Создаёт переадресованное сообщение. |
...
| [setMessageExtensions](https://trtc.io/document/54166?platform=javascript&product=chat&menulabel=sdk#5243dc69-bf46-49a0-8c21-b7e7f3bbdb0e) | Устанавливает расширения сообщений. |
| [getMessageExtensions](https://trtc.io/document/54166?platform=javascript&product=chat&menulabel=sdk#.E8.8E.B7.E5.8F.96.E6.B6.88.E6.81.AF.E6.89.A9.E5.B1.95) | Получает расширения сообщений. |
| [deleteMessageExtensions](https://trtc.io/document/54166?platform=javascript&product=chat&menulabel=sdk#9c2c4ca3-3f4d-4c33-86ba-f7000c2bb7e7) | Удаляет расширения сообщений. |
| [addMessageReaction](https://trtc.io/document/60749?platform=javascript&product=chat&menulabel=sdk#07af1608-075c-4f2b-84e8-48987cd2ee4e) | Добавляет реакции на сообщения. Этот интерфейс позволяет реализовать сценарии, такие как "реакции эмодзи". |
| [removeMessageReaction](https://trtc.io/document/60749?platform=javascript&product=chat&menulabel=sdk#82baa97c-bf9e-4732-a9bd-29c05f18a930) | Удаляет реакции на сообщения. |
| [getMessageReactions](https://trtc.io/document/60749?platform=javascript&product=chat&menulabel=sdk#2ef2bd48-285f-4f4c-9e3e-662e3cf74330) | Получает информацию о реакциях на сообщения. |
| [getAllUserListOfMessageReaction](https://trtc.io/document/60749?platform=javascript&product=chat&menulabel=sdk#c4cf8036-a7ab-4e1f-9c51-01cf44b2ef41) | Постранично получает список пользователей для определённой реакции на сообщение. |
...
| [getMessageListHopping](https://trtc.io/document/47999?platform=javascript&product=chat&menulabel=sdk#a9fd17b1-21b0-4898-9d8f-1fa3c978e053) | Извлекает список сообщений диалога по указанной последовательности или диапазону времени. |
| [sendMessageReadReceipt](https://trtc.io/document/48021?platform=javascript&product=chat&menulabel=sdk#sending-a-message-read-receipt-(by-the-receiver)) | Отправляет подтверждения о прочтении сообщений. |
| [getMessageReadReceiptList](https://trtc.io/document/48021?platform=javascript&product=chat&menulabel=sdk#pulling-message-read-receipt-information-(by-the-sender)) | Получает список подтверждений о прочтении сообщений. |
| [getGroupMessageReadMemberList](https://trtc.io/document/48021?platform=javascript&product=chat&menulabel=sdk#pulling-the-list-of-members-who-have-or-have-not-read-a-group-message-(by-the-sender)) | Получает список участников, которые прочитали (или не прочитали) групповое сообщение. |
...
| [setConversationDraft](https://trtc.io/document/56184?platform=javascript&product=chat&menulabel=sdk#setting-a-conversation-draft) | Установить черновик диалога. |
...
| [pinConversation](https://trtc.io/document/48316?platform=javascript&product=chat&menulabel=sdk#pinning.2Funpinning-a-conversation-to.2Ffrom-the-top) | Закрепляет/Открепляет диалог вверху списка. |
...
| [searchCloudGroupMembers](https://www.tencentcloud.com/document/product/1047/67683#b57f11f2-d80b-4e40-b9e3-8f9463195624) | Поиск участников облачных групп. |
...
| [getBlacklist](https://trtc.io/document/48152?platform=javascript&product=chat&menulabel=sdk#07e8dcd3-0bca-411f-97a0-598760b76ee6) | Получает вашу чёрный список. |
| [addToBlacklist](https://trtc.io/document/48152?platform=javascript&product=chat&menulabel=sdk#blocking-a-user) | Добавляет пользователя в чёрный список. |
| [removeFromBlacklist](https://trtc.io/document/48152?platform=javascript&product=chat&menulabel=sdk#unblocking-a-user) | Удаляет пользователя из чёрного списка. |
...
| API | Описание |
| --- | --- |
| [getFriendList](https://trtc.io/document/48158?platform=javascript&product=chat&menulabel=sdk#getting-contacts) | Получает контакты из кэша SDK. |
...
| [getFriendApplicationList](https://trtc.io/document/48158?platform=javascript&product=chat&menulabel=sdk#cfc251a7-01f1-430b-81d1-c60fe9e0da86) | Получает список запросов дружбы из кэша SDK. |
...
| [getFriendGroupList](https://trtc.io/document/48155?platform=javascript&product=chat&menulabel=sdk#getting-a-friend-list) | Получает список групп друзей из кэша SDK. |
...
| [renameFriendGroup](https://trtc.io/document/48155?platform=javascript&product=chat&menulabel=sdk#renaming-a-friend-list) | Изменяет название группы друзей. |
...
| [changeGroupOwner](https://trtc.io/document/48465?platform=javascript&product=chat&menulabel=sdk#transferring-a-group) | Передаёт владение группой. |
| [getGroupApplicationList](https://trtc.io/document/48465?platform=javascript&product=chat&menulabel=sdk#c69d

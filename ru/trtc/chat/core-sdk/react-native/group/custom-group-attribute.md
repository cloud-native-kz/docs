...
С помощью групповых атрибутов вы можете управлять местами в аудиочатах. Когда пользователь включает микрофон, вы можете установить групповой атрибут для управления информацией пользователя. Когда пользователь выключает микрофон, вы можете удалить групповой атрибут. Другие члены группы могут получить список групповых атрибутов для отображения списка мест.

> **Примечание:** Тема не поддерживает групповые атрибуты.

Функция группового атрибута имеет следующие характеристики:

1. Вы можете настроить до 16 групповых атрибутов. Размер каждого группового атрибута может быть до 4 КБ, а общий размер всех групповых атрибутов может быть до 16 КБ. Когда вы впервые изменяете групповые атрибуты после успешного входа, вызовите `getGroupAttributes` для получения последних групповых атрибутов перед тем, как снова инициировать операцию изменения.
2. Когда несколько пользователей одновременно изменяют одни и те же групповые атрибуты, успешно выполнить операцию может только первый пользователь, остальные пользователи получат код ошибки 10056. После получения этого кода ошибки вызовите `getGroupAttributes` для обновления локально сохраненных групповых атрибутов до последней версии перед тем, как снова инициировать операцию изменения.
3. Прежде чем использовать эту функцию в AVChatRoom после успешного входа, необходимо вызвать API `joinGroup` для присоединения к аудио-видео группе. Для публичной группы (Public), группы собраний (Meeting), рабочей группы (Work) и группы сообщества (Community) повторное присоединение к группе не требуется.
...
---
*Источник: [https://trtc.io/document/50408](https://trtc.io/document/50408)*

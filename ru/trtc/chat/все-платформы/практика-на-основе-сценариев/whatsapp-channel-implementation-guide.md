# Руководство по внедрению канала WhatsApp

Наше официальное решение для канала предоставляет услуги коммуникации на основе подписки и распространения контента, которые аналогичны возможностям канала WhatsApp. Благодаря интегрированным API предприятия могут создавать и управлять проверенными организационными учетными записями; публиковать статьи, мультимедийные сообщения и интерактивный контент; взаимодействовать с подписчиками, а подписчики также могут отправлять сообщения на официальный канал; отслеживать операционные метрики через REST API и системы обратных вызовов от третьих сторон.

**Сценарии использования:**

Каналы, принадлежащие бренду, на WhatsApp Business, которые обеспечивают прямую коммуникацию с клиентами посредством мультимедийных сообщений (текст, изображения, видео) и интеграции служб. Ниже приведены три распространенных сценария использования нашего официального канала:

- Трансляция информации в реальном времени: прямая доставка новостей, учебного контента и обновлений в ленты пользователей.
- Повышение узнаваемости бренда: развитие узнаваемости и влияния посредством тщательно отобранных мультимедийных сообщений.
- Интеграция служб: включение транзакций (например, банковских переводов).

![](https://cloudcache.intl.tencent-cloud.com/cms/backend-cms/d2e377dd483211f0930e525400bf7822.png)

Решение официального канала помогает бизнесу генерировать доход от своих официальных учетных записей, оптимизируя весь процесс — от привлечения пользователей социальных сетей до установления прямых каналов коммуникации и, в конечном итоге, преобразования их в выручку от продаж.

## Обзор

Официальный канал может отправлять трансляционные сообщения подписавшимся пользователям, а также вести с ними одиночные чаты.

При обмене сообщениями создается одиночный [разговор](https://trtc.io/document/48329?platform=android&product=chat&menulabel=sdk) со структурой разговора ID как c2c_officialAccountID.

Для функций управления, таких как создание официального канала, см. [API серверной части](https://www.tencentcloud.com/document/product/1047/60755). IMSDK в основном предоставляет функциональность подписки на официальный канал, отписки от официального канала и получения списка официальных каналов.

> **Примечание:** эта функция поддерживается только выпуском Enhanced от версии 7.6.5011 или выше.

## Введение в класс профиля официального канала

| Атрибут | Определение | Описание |
| --- | --- | --- |
| officialAccountID | ID официального канала | ID официального канала должен быть префиксирован с @TOA#_, может быть [настроен](https://www.tencentcloud.com/document/product/1047/60813) и имеет максимальную длину 48 байт. |
...
| organization | имя организации | Максимальная длина: 500 байт (кодировка UTF-8, где 1 китайский символ занимает 3 байта) |
...
## Получение списка официальных каналов
...
- Когда officialAccountIDList пусто, возвращает список подписанных официальных каналов.
- Когда в officialAccountIDList предоставлены конкретные ID официальных каналов, возвращает информацию для этих указанных официальных каналов.
...
```
[[V2TIMManager sharedInstance] getOfficialAccountsInfo:nil succ:^(NSArray<V2TIMOfficialAccountInfoResult *> *resultList) {    [self appendString:[NSString stringWithFormat:@"успех：%@", resultList]];} fail:^(int code, NSString *desc) {    [self appendString:[NSString stringWithFormat:@"ошибка：код：%d сообщение：%@",code,desc]];}];
```
...
## Обмен сообщениями через официальный канал

Официальные каналы поддерживают два типа сообщений:

1. Трансляционные сообщения — отправляются всем подписчикам.
2. Одиночные сообщения — приватные разговоры между официальным каналом и отдельными подписчиками.

### Отправка трансляционных сообщений

Используйте [API трансляционных сообщений серверной части](https://www.tencentcloud.com/document/product/1047/60810) для отправки сообщений всем подписчикам официального канала.

### Обмен одиночными сообщениями
...
Используйте [API одиночных сообщений серверной части](https://www.tencentcloud.com/document/product/1047/34919), указав:
...
---
*Источник: [https://trtc.io/document/69925](https://trtc.io/document/69925)*

---
*Источник (EN): [whatsapp-channel-implementation-guide.md](./whatsapp-channel-implementation-guide.md)*

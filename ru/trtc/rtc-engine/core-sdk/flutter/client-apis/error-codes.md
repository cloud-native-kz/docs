...
`TRTCCloud.enterRoom()` выдает этот тип кода ошибки при неудачном входе в комнату. Вы можете использовать функции обратного вызова `TRTCCloudDelegate.onEnterRoom()` и `TRTCCloudDelegate.OnError()` для захвата связанных уведомлений.
...
`TRTCCloud.exitRoom()` выдает эту ошибку при неудачном выходе из комнаты. Вы можете использовать функцию обратного вызова `TRTCCloudDelegate.OnError()` для захвата связанных уведомлений.
...
Вы можете использовать функцию обратного вызова `TRTCCloudDelegate.OnError()` для захвата связанных уведомлений.
...
Вы можете использовать функции обратного вызова `TRTCCloudDelegate.onStartPublishing()` и `TRTCCloudDelegate.onSetMixTranscodingConfig()` для захвата связанных уведомлений.
...
| ERR_ROOM_REQUEST_START_PUBLISHING_TIMEOUT | -3333 | Signaling of starting to push to Tencent Cloudâs live streaming CDN timed out. |
| ERR_ROOM_REQUEST_START_PUBLISHING_ERROR | -3334 | Abnormal signaling of starting to push to Tencent Cloudâs live streaming CDN. |
| ERR_ROOM_REQUEST_STOP_PUBLISHING_TIMEOUT | -3335 | Signaling of stopping pushing to Tencent Cloudâs live streaming CDN timed out. |
| ERR_ROOM_REQUEST_STOP_PUBLISHING_ERROR | -3336 | Abnormal signaling of stopping pushing to Tencent Cloudâs live streaming CDN. |

### Коды ошибок межкомнатного взаимодействия

`TRTCCloud.ConnectOtherRoom()` выдает этот тип кода ошибки при неудачном кросс-комнатном взаимодействии. Вы можете использовать функцию обратного вызова `TRTCCloudDelegate.onConnectOtherRoom()` для захвата связанных уведомлений.
